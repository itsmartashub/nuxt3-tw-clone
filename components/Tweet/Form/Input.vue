...
const isDisabled = computed(() => (text.value === '' ? true : false))
...
function handleImageClick() {
	imageInput.value.click() // 💥💥💥💥 simuliramo click. Ovo je input polje za biranje fajla tj Choose file. Ali smo ga sakrili, i simulirali isti klik, ali na ikonici toj. Zato nam sluzi ono ref="imageInput"
}

function handleImageChange(e) {
	// ovo se okine kada izaberemo sliku pa kazemo mu: 'open'
	// console.log(e)
	const file = e.target.files[0] // zelimo da dohvatimo samo prvi fajl
	selectedFile.value = file

	const reader = new FileReader()

	// sad trebad a implementiramo prikaz slike kad je ucitamo tj uploadujemo sa choose file --> open. Koristicemo js reader
	reader.onload = (e) => {
	...
	<div>
		<div class="flex items-center flex-shrink-0 p-4 pb-0">
			<div class="flex w-12 items-top">
				<img class="inline-block w-10 h-10 rounded-full" :src="props.user?.profileImage" alt="" />
			</div>
		...
		<div class="flex p-2 pl-14">
			...
			<div class="ml-auto">
				<UIButton size="sm" :disabled="isDisabled" @click="handleFormSubmit">
